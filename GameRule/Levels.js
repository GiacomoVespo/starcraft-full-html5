var Levels=[
    {
        ...
        load:function(){
			...
			new Audio(Game.CDN+'bgm/soundtrack/Background.wav').play();
        }
    },
    {
        level:2,
		label:'Zerg VS Terran',
        load:function(){
            //Test multiplayer
    ...
    },
    {
        ...
        load:function(){
			...
			if (!Game.replayFlag) Game.team=2;
            Game.race.choose(races[Game.team]);
            Map.offsetX=startPoint[Game.team].x;
            Map.offsetY=startPoint[Game.team].y;
            //Apply cheat
			...
			//news by Giacomo Vespo: Information by buildings and research
			var speech=[
				"ENEMY ENGAGE! Battle Coming! ",
                "Please, Supply Depots need to be built by SCVs!",
                "Before you build, do your research!",
                "Upgrade Defense and Attack Power!",
                "Marine and Ghost are needed!",
                "Let's see if you are a real commander! More Battle Cruisers needed!",
                "Don't forget the ground tanks!",
                "Are you getting soft? Get busy!",
                "Remember you have 3 enemies to face! Tactical Scheme!",
                "Have you lined up the teams?",
                "Come on! We need to scout the surrounding area!",
            ...
            for (var N=0;N<speech.length;N++){
                (function(n){
                    Game.commandTimeout(function(){
                        Game.showMessage('Command Center: '+speech[n],5000);
                    },n*30000+10000);
                })(N);
            }
			//by Giacomo Vespo: Enemy attack at Command Center
				
			var offsets=[{x:2794,y:244},{x:2138,y:3675},
                {x:464,y:2568},{x:2086,y:486}];
            var num=0, wave=1;
            var interval=120000;//120 seconds per wave
            _$.traverse([Neutral,Zerg,Protoss],function(enemyType){
                Game.commandTimeout(function(){
                    offsets.forEach(function(offset){
                        offset.team=1;
                        new enemyType(offset).attackGround({x:320,y:180});
						Resource[Game.team].mine+=30;
						Resource[Game.team].gas+=30;
                    });
